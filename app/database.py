...
class DonorDetail(db.Model):
    ...
    personal_details_id = db.Column(db.Integer, db.ForeignKey('PersonalDetailsUser.id'), nullable=False)
    address_id = db.Column(db.Integer, db.ForeignKey('AddressDetailsUser.id'), nullable=False)
    active_status = db.Column(db.Boolean, nullable=False, default=True)
    disease_id = db.Column(db.Integer, db.ForeignKey('DiseaseDetailsUser.id'))
...
class AuthenticationDetailsAdmin(db.Model):
    ...
    admin_id = db.Column(db.Integer, db.ForeignKey('AdminDetails.id'), primary_key=True)
    name = db.Column(db.String(100), nullable=False)
...
class BloodRequestDetails(db.Model):
    ...
    hospital_id = db.Column(db.Integer, db.ForeignKey('HospitalDetails.id'), nullable=False)
    contact_number = db.Column(db.String(15), nullable=False)
    ...
    response_id = db.Column(db.Integer, db.ForeignKey('ResponseDetails.id'))
...
class ResponseDetails(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    blood_request_id = db.Column(db.Integer, db.ForeignKey('BloodRequestDetails.id'), nullable=False)
    status = db.Column(db.String(45), nullable=False)
